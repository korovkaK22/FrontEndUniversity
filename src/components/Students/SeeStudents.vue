<template>
  <div class="Vue">
    ...
    <div v-if="id!==0">
      ...
      <div class=titleText>Студент <span class=italic><br>" {{ data.name }} "  </span></div>

      <div class=description>
        Ім'я: <span class=italic> {{ data.name }}; </span><br>
        Прізвище:  <span class=italic> {{ data.surname }};  </span> <br>
        Почта: <span class=italic> {{ data.email }}; </span><br>
        Номер телефону:  <span class=italic> {{ data.phone }};  </span> <br>
        Група:  <span class=italic>  <router-link v-bind:to="('/seeGroups/?id='+data.group_id)">
        {{ groups.name }};
        </router-link>  </span> <br>
